...
Console &Console::return_impl(const SHORT &row, const SHORT &col, const std::string_view str, const ansiColor &color,
                              const ansiBackground &background, const bool &needsRevisited) noexcept {
...
Console &Console::return_impl_ln(const SHORT &row, const std::string_view str, const ansiColor &color,
                                 const ansiBackground &background, const bool &needsRevisited) noexcept {
    const SHORT preY = cursorCoordinate.Y;
    const SHORT preX = cursorCoordinate.X;
...
//void Console::printAndEraseMessage(const std::string_view str, const short prevY, const short prevX, 
//                                   const ansiColor &color, const ansiBackground &background) {
//    std::thread([=, this]() {
//        {
//            std::lock_guard<std::mutex> lock(cursorMutex_);
//            // Set cursor position if necessary
//            // std::cout << "\033[" << prevY << ";" << prevX << "H";
//            bot(str, color, background);
//        }
//
//        std::this_thread::sleep_for(std::chrono::seconds(3));
//
//        {
//            std::lock_guard<std::mutex> lock(cursorMutex_);
//            // Clear the message by overwriting it with spaces
//            // Assuming your console class has a way to move the cursor
//            // std::cout << "\033[" << prevY << ";" << prevX << "H";
//            bot(std::string(str.length(), ' '), ansiColor::green, ansiBackground::black);
//        }
//    }).detach();
//}
